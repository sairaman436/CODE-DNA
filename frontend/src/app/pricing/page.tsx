        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-20">
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-[11px] uppercase tracking-[0.2em] font-bold text-emerald-500/70">Pricing</span>
              <div className="h-px w-8 bg-emerald-500/50" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/[0.04] rounded-2xl overflow-hidden">
            {PLANS.map((plan, i) => (
              <motion.div
                ...
                className={`bg-zinc-950 p-8 flex flex-col ${plan.highlight ? 'ring-1 ring-emerald-500/20 ring-inset relative' : ''}`}
              >
                {plan.highlight && (
                  <div className="absolute top-0 inset-x-0 h-px bg-emerald-500/40" />
                )}
                <div className="mb-8">
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-[12px]">
                      <Check className="w-3.5 h-3.5 text-emerald-500/60 mt-0.5 shrink-0" />
