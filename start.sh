...
(cd "$ROOT_DIR/engine" && python -m uvicorn main:app --port 8000) &
ENGINE_PID=$!
(cd "$ROOT_DIR/engine" && python -m uvicorn main:app --port 8001) &
ENGINE_PID_2=$!
(cd "$ROOT_DIR/engine" && python -m uvicorn main:app --port 8002) &
ENGINE_PID_3=$!
